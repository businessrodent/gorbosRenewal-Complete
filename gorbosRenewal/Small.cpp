#include <iostream>
#include <vector>
using namespace std; int i = 0; int gold = 80; struct Item {string name; int val; int nom; string desc;	Item(string name, int val, int nom, string desc) {this->name = name; this->val = val; this->nom = nom; this->desc = desc;}}; struct Inventory {	vector<Item> list; void add(Item item) {bool y = 0; for (Item& x : list) {if (x.name == item.name) {y = 1; x.nom++; break;}}; if (y == 0) {Item b = item; b.nom = 1; list.push_back(b);}}; void sub(Item item) { bool y = 0; int n = 0; for (Item& x : list) {if (x.name == item.name) {if (x.nom > 1) {x.nom--;} else {list.erase(list.begin() + n);}}	n++;};}; Inventory(vector<Item> list) {this->list = list;}Inventory() {	}};Inventory shop, inv;void shopBuy() {int i = 0; int inp; cout << "shop: \n"; for (const Item& item : shop.list) {i++;	cout << i << ". " << item.name << " x" << item.nom << " - price: " << item.val << "\n";}cout << shop.list.size() + 1 << ". back \n";while (1) {cin >> inp; if (cin.fail()) {cin.clear();cin.ignore(256, '\n');}	if (0 < inp && inp <= shop.list.size() + 1) {break;}cout << "invalid \n";}if (inp < shop.list.size() + 1) {if (gold >= shop.list[inp - 1].val) {gold -= shop.list[inp - 1].val;inv.add(shop.list[inp - 1]); shop.sub(shop.list[inp - 1]);} else {cout << "not enough gold \n";}}};
void shopSell() {
	int i = 0;
	int inp;
	cout << "inventory: \n";
